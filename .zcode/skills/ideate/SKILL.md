...
**story-incubate(孵化)** → **genre-playbook(流派定调第一叶: 选流派+装引擎+读者契约,genre_contract.py机器门,2026-09-15)** → **premise-fit(适配闸:AI擅长×语料富度×概念维持成本×受众主流×长跑空间,≥20/25;重生穿越流另有三硬前置)** → **era-goldmine(重生/穿越书必走: 矿产档案+限制器,无档案拒开卷)** → premise → theme-dossier → world-rules → world-power → world-map → world-economy → world-history → world-culture → iceberg-budget → char-bible → char-web → char-voice → plot-spine → romance-line → volume-outline → set-piece → opening-arc → unit-designer → foreshadow-plan → reward-economy → pacing-score → thread-weaver → positioning → ideate-audit → **trial-write(试写验证)** (naming按需插队;收官卷排纲→**finale-engineering**(完本收束工程,红队20260919商业批))
...
## 设计产物分层（红队20260919滚动前瞻落地）

| 层 | 产物 | 变更规则 |
|---|---|---|
| 骨架层(锁死) | 前提句/终卷答案/人物核心弧线/名场面 | 第一卷锁定后不改(红队致命伤除外) |
| 卷纲层(每卷重排) | 各卷纲/伏笔埋收/冲突升级 | 卷末复盘后根据实际重写 |
| 章卡层(前瞻5-10章) | 场景卡/beats/钩型 | 写前5-10章细化,不更远 |
| 即兴层(完全自由) | 对话细节/角色即兴/闲笔 | 写作时完全自由,事后回填卡 |

## 卷纲滚动纪律（补充上述分层表）
- 卷纲只细化当前卷+下一卷,不做更远(远处留骨架)
- 卷末复盘后,下一卷纲必须根据arc-review+漂移审计数据重写
- 场景卡前瞻窗口=5-10章(next~next+9),跨卷须先有下一卷纲
