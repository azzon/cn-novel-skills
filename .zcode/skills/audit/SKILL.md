...
`- <日期> | audit | 用户诉求一句话 | 选中:<技能名> | 理由一句`
**判据(可审计)**: ①选中技能须与最终产物对得上(账上选A产物却是B=路由错,记入修订期) ②同一诉求三次路由到不同技能=入口判据不清,回本入口补判据表 ③无路由账行的产物=绕过入口直呼技能,pre-commit指纹门之外的流程债。

路由表: 单场景→scene-audit; 章冷读→reader-proxy; 卷连读→arc-review; 一致性→consistency-audit; 人物→character-audit; 漂移→drift-audit; 伏笔→foreshadow-audit; 喜剧→humor-audit; 知情→knowledge-audit。
