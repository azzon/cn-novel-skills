...
def main():
    ...
    for p in staged:
        ...
        if mode == "new":
            ...
            for _l in _lines:
                if not re.match(r"^第[一二三四五六七八九十百0-9]+章", _l.strip()):
                    first_para = _l.strip()
                    break
            if len(first_para) > 20 and re.match(r"^(第?[一二三四五六七八九十百0-9]+[章日天早晚月年]|开春|进了腊月|正月|入了|那年|当年|次日|第二天|当天|礼拜|周[一二三四五六日末]|深夜|凌晨|傍晚|天黑|十月|十一月|十二月|三月)", first_para):
        ...
        _bk1 = book_root(p)
        _bp = _bk1 / "商业计划.md"
        if _bp.exists() and re.search(r"^平台[:：]\s*番茄", _bp.read_text(encoding="utf-8"), re.M):   # W6验证:原search全文,竞品对比提及即误降
                ...
                if _wv.exists():
                    for _l in _wv.read_text(encoding="utf-8").splitlines():
                        ...
                        if _re.match(rf"- ch0*{n}:", _l.strip()):
                            if _re.search(r"(^|[;:，,\s])(g1|check|all)([;:)，,\s]|$)", _l):   # W6验证:原子串search,g1命中g10、理由含check误开
                                _wv_ok = True
                            break
                if _wv_ok:
