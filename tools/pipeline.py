...
PREFIX = (
    ...
    "禁工程词;情绪禁告知(很震惊=写行为);明喻<=3,破折号<=3,警句<=1/场景;"
    "首句禁时间状语开场(与前两章错型)。"
    "【生活气】金额>=2(1处参与情绪运算),感官>=3通道,闲笔>=1(>=60字),每章1轮家常对话。"
    "【情绪纪律】高情感拍减速到秒级三连微拍,单拍<=15字。"
    "【段落形态】段均<=30字;长段>=110字全章<=3;每场景3个<=15字短段;峰章=卡带中位x1.15。"
    "【章末】必钩,形态与上两章轮换;警句式收尾每卷<=1/3;章末禁旁白判词。"
    "【禁宣言】章末禁排比宣言/自觉宣言(他不是那种X)/作者总结;收在动作/物件/对话,不收在道理。"
    "【防同构】对白禁复盘腔;讲解问答每章<=1次且提问人轮换;章末禁议论;"
    "本章至少一处失控/失败/意外;摘要句仅限过渡。"
    "【对白声口】漏不要聚:废话率>=30%,句碎片化,语气词>=8/千字,答非所问>=15%,埋怨代哲理;配角警句同章<=1枚。"
    "【禁词】写对白前查声口卡:该角色禁词绝不入其台词;口头禅每3-4次出场落1次;遮名可辨靠句式/词汇/废话习惯。"
    "【数字表】卡上数字逐一入正文,口语可(百分之三)数值不可变;修订漂移=对账FAIL。"
