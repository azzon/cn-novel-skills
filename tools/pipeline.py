...
PREFIX = (
    "【生成纪律】生成单位=一个场景,目标3500-5000字。对话40-60%(角色必须开口),心理活动>=3处/千字。"
    "禁工程词,禁情绪告知(他很震惊=写行为),明喻<=3,破折号<=3,警句<=1/场景,"
    "首句禁时间状语开场(用对话/动作/异常直入,与前两章错型)。"
    "【生活气硬指标】金额>=2(1处参与情绪运算),感官>=3通道,温度实感>=2,闲笔>=1(>=60字),"
    "等待给可感刻度,每章1轮家常对话。"
    "【情绪纪律】情绪禁直接命名;高情感拍减速到秒级三连微拍,单拍<=15字。"
    "【段落形态硬预算】段均<=30字;>=110字长段全章<=3;每场景至少3个<=15字短段;峰章=卡带中位x1.15。"
    "【章末】必钩,形态与上两章轮换,每卷警句式收尾<=1/3,章末禁旁白判词。"
    "【防同构】对白禁复盘腔;讲解问微结构每章<=1次且提问人轮换;章末禁议论收尾;"
    "本章至少一处失控/失败/意外(零失败叙事=机器指纹);摘要句压缩仅限过渡,禁章末。"
    "【对白声口】对白要漏不要聚:废话率>=30%,句碎片化,语气词密度>=8/千字,答非所问>=15%,埋怨代哲理。配角警句同章合计<=1枚。"
    "【峰后禁释令】情感峰值段(哭/怒/喜/疼/暖)后,下一段必须是动作/物件/沉默/环境。禁叙述者解释(想明白了/这叫做XX/有一种账记不进报表/原来如此)。峰后必释=杀掉心头一紧。"
    "每个场景必须有人想要不同的东西(阻碍)。没有阻碍=没有场景=删掉重写。只写本卡;卡上Forbid绝对不写;价值换极必须发生;对白遮名可辨。"
)
